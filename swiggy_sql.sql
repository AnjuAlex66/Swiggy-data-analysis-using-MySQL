...
select  Restaurant,city, max(Delivery_time) from swiggy_data 
group by Restaurant,city 
order by max(Delivery_time) desc limit 28 ;
-- 28 restaurants  are from the city kolkata 
 
select  Restaurant,city, min(Delivery_time) from swiggy_data 
group by Restaurant,city order by min(Delivery_time)  limit 10;
...
create view City_Counts as select City,count(City) from swiggy_data
 group by City;
...
-- 10. which restaurant has more rating in each city?

select city,restaurant, avg_ratings from swiggy_data
having Avg_ratings=(select max(Avg_ratings) from swiggy_data) 
order by City; 

-- 11. Number of restaurants in each city serving indian food

select city, count(Restaurant) from swiggy_data  
where Food_type like 'indian'group by city  
order by count(Restaurant) desc  ;
-- Kolkata at the top and surat at the last 

-- 12.  Average delivery time by city

select city,round(avg(Delivery_time),0)  as avg_delivery_time from swiggy_data 
group by city
